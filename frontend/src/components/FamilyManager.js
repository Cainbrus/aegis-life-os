...
const ROLE_META = {
  ...
  limited: { label: 'Limited', color: 'text-blue-400', Icon: UserCog },
    ...
    <div className="p-5 space-y-5" data-testid="family-screen">
      <div>
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          <Users className="text-blue-400" size={26} /> Trusted Family
        </h2>
      ...
      <div className="space-y-2" data-testid="family-list">
        {profiles.map((p) => {
          ...
          return (
            <div key={p.id} className="flex items-center gap-3 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3.5" data-testid="family-member">
      ...
      {!adding ? (
        ...
        <div className="rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 p-4 space-y-3" data-testid="family-add-form">
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" data-testid="family-name"
            className="w-full px-4 py-3 rounded-xl bg-[#0e1626] border border-slate-600 text-white outline-none focus:border-blue-500" />
            ...
            {ROLES.map((r) => {
              ...
              return (
                <button key={r.id} onClick={() => setRole(r.id)} data-testid={`family-role-${r.id}`}
                  className={`py-3 rounded-xl border flex flex-col items-center gap-1 ${sel ? 'border-blue-500 bg-blue-500/10 text-blue-400' : 'border-slate-700 bg-[#0e1626] text-slate-300'}`}>
          ...
          <input value={recovery} onChange={(e) => setRecovery(e.target.value)} placeholder="Your Recovery code (to authorise)" type="password" data-testid="family-recovery"
            className="w-full px-4 py-3 rounded-xl bg-[#0e1626] border border-slate-600 text-white outline-none focus:border-blue-500" />
          <button onClick={add} disabled={busy} data-testid="family-save" className="w-full py-3 rounded-xl bg-blue-500 text-slate-900 font-bold disabled:opacity-50">
